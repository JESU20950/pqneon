...
/* see api.h */
int
PQCLEAN_FALCON512_NEON_crypto_sign_keypair(unsigned char *pk, unsigned char *sk) {
    union {
    ...
    PQCLEAN_FALCON512_NEON_keygen(&rng, f, g, F, NULL, h, 9, tmp.b);
    ...
    sk[0] = 0x50 + 9;
    u = 1;
    v = PQCLEAN_FALCON512_NEON_trim_i8_encode(
            sk + u, PQCLEAN_FALCON512_NEON_CRYPTO_SECRETKEYBYTES - u,
            f, 9, PQCLEAN_FALCON512_NEON_max_fg_bits[9]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = PQCLEAN_FALCON512_NEON_trim_i8_encode(
            sk + u, PQCLEAN_FALCON512_NEON_CRYPTO_SECRETKEYBYTES - u,
            g, 9, PQCLEAN_FALCON512_NEON_max_fg_bits[9]);
    if (v == 0) {
        return -1;
    }
    u += v;
    ...
    u += v;
    if (u != PQCLEAN_FALCON512_NEON_CRYPTO_SECRETKEYBYTES) {
        return -1;
    }
     ...
     */
    pk[0] = 0x00 + 9;
    v = PQCLEAN_FALCON512_NEON_modq_encode(
            pk + 1, PQCLEAN_FALCON512_NEON_CRYPTO_PUBLICKEYBYTES - 1,
            h, 9);
    if (v != PQCLEAN_FALCON512_NEON_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    u = 1;
    v = PQCLEAN_FALCON512_NEON_trim_i8_decode(
            f, 9, PQCLEAN_FALCON512_NEON_max_fg_bits[9],
            sk + u, PQCLEAN_FALCON512_NEON_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    ...
    if (!PQCLEAN_FALCON512_NEON_complete_private(G, f, g, F, 9, tmp.b)) {
        return -1;
    }
    ...
    PQCLEAN_FALCON512_NEON_hash_to_point_ct(&sc, r.hm, 9, tmp.b);
     ...
     */
    for (;;) {
        PQCLEAN_FALCON512_NEON_sign_dyn(r.sig, &sc, f, g, F, G, r.hm, 9, tmp.b);
        v = PQCLEAN_FALCON512_NEON_comp_encode(sigbuf, *sigbuflen, r.sig, 9);
        if (v != 0) {
            inner_shake256_ctx_release(&sc);
            *sigbuflen = v;
            return 0;
...
do_verify(
    ...
    if (PQCLEAN_FALCON512_NEON_modq_decode(h, 9,
                                           pk + 1, PQCLEAN_FALCON512_NEON_CRYPTO_PUBLICKEYBYTES - 1)
            != PQCLEAN_FALCON512_NEON_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
    PQCLEAN_FALCON512_NEON_to_ntt_monty(h, 9);
    ...
    if (PQCLEAN_FALCON512_NEON_comp_decode(sig, 9, sigbuf, sigbuflen) != sigbuflen) {
        return -1;
    }
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    PQCLEAN_FALCON512_NEON_hash_to_point_ct(&sc, hm, 9, tmp.b);
    inner_shake256_ctx_release(&sc);

    /*
     * Verify signature.
     */
    if (!PQCLEAN_FALCON512_NEON_verify_raw(hm, sig, h, 9, tmp.b)) {
        return -1;
    }
    return 0;
}
...
/* see api.h */
int
PQCLEAN_FALCON512_NEON_crypto_sign_signature(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    /*
     * The PQCLEAN_FALCON512_NEON_CRYPTO_BYTES constant is used for
     * the signed message object (as produced by PQCLEAN_FALCON512_NEON_crypto_sign())
     * and includes a two-byte length value, so we take care here
     * to only generate signatures that are two bytes shorter than
     * the maximum. This is done to ensure that PQCLEAN_FALCON512_NEON_crypto_sign()
     * and PQCLEAN_FALCON512_NEON_crypto_sign_signature() produce the exact same signature
    ...
    vlen = PQCLEAN_FALCON512_NEON_CRYPTO_BYTES - NONCELEN - 3;
...
/* see api.h */
int
PQCLEAN_FALCON512_NEON_crypto_sign_verify(
...
/* see api.h */
int
PQCLEAN_FALCON512_NEON_crypto_sign(
    ...
    pm = sm + 2 + NONCELEN;
    sigbuf = pm + 1 + mlen;
    sigbuflen = PQCLEAN_FALCON512_NEON_CRYPTO_BYTES - NONCELEN - 3;
...
/* see api.h */
int
PQCLEAN_FALCON512_NEON_crypto_sign_open(
